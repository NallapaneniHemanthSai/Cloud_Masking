"""Milestone 2 verification: project directory structure exists.

These tests assert the scaffold is present and correctly shaped. They must pass on a bare Python 3.11
interpreter with **no third-party packages installed**.
"""

from __future__ import annotations

from pathlib import Path

import pytest

PROJECT_ROOT = Path(__file__).resolve().parents[2]

# Directories that must exist after Milestone 2.
REQUIRED_DIRS = [
    "backend/app/api/routers",
    "backend/app/core",
    "backend/app/models",
    "backend/app/services",
    "backend/app/preprocessing",
    "backend/app/inference",
    "backend/app/training",
    "backend/app/evaluation",
    "backend/app/comparison",
    "backend/app/acceptance",
    "backend/app/datasets",
    "backend/app/change_detection",
    "backend/app/db",
    "backend/app/schemas",
    "backend/app/visualization",
    "backend/app/utils",
    "backend/tests",
    "backend/configs",
    "backend/scripts",
    "frontend/src/components",
    "frontend/src/pages",
    "frontend/src/services",
    "frontend/src/hooks",
    "frontend/src/utils",
    "frontend/src/assets",
    "docker",
    "docs",
    "data/raw",
    "data/processed",
    "data/samples",
    "models",
    "models/checkpoints",
    "notebooks",
    "outputs",
    "experiments",
    "data/raw/cloudsen12",
    "data/raw/on_cloud_n",
    "data/external",
    "data/manifests",
    "data/metadata",
    "docs/datasets",
    "docs/deployment",
    "docs/install",
    "docs/user_guide",
    "docs/developer_guide",
    "docs/api",
    ".github/workflows",
]

# Individual files that must exist after Milestone 2.
REQUIRED_FILES = [
    "backend/pyproject.toml",
    "backend/requirements.in",
    "backend/requirements-dev.in",
    "backend/.env.example",
    "backend/README.md",
    "backend/configs/config.template.yaml",
    "backend/configs/logging.yaml",
    "backend/app/__init__.py",
    "backend/app/main.py",
    "backend/app/core/config.py",
    "backend/app/core/constants.py",
    "backend/app/core/exceptions.py",
    "backend/app/core/logging_config.py",
    "backend/app/schemas/__init__.py",
    "backend/app/datasets/manifest.py",
    "backend/app/datasets/integrity.py",
    "backend/app/datasets/download.py",
    "backend/app/datasets/verification.py",
    "backend/app/datasets/experimental_config.py",
    "backend/app/datasets/availability.py",
    "backend/app/datasets/records.py",
    "backend/app/datasets/validation_gates.py",
    "backend/app/datasets/sampling.py",
    "backend/app/datasets/dataset_statistics.py",
    "backend/app/datasets/artifact.py",
    "backend/app/datasets/readiness.py",
    "backend/app/datasets/pipeline.py",
    "backend/app/datasets/synthetic.py",
    "backend/app/datasets/cloudsen12_access.py",
    "backend/app/core/telemetry.py",
    "backend/app/db/base.py",
    "backend/app/db/models.py",
    "backend/app/schemas/api.py",
    "backend/app/inference/predictor.py",
    "backend/app/services/system_service.py",
    "backend/app/services/model_service.py",
    "backend/app/services/training_service.py",
    "backend/app/services/prediction_service.py",
    "backend/app/services/evaluation_service.py",
    "backend/app/services/history_service.py",
    "backend/app/services/upload_service.py",
    "backend/app/services/lineage_service.py",
    "backend/app/services/integration_service.py",
    "backend/app/acceptance/__init__.py",
    "backend/app/acceptance/records.py",
    "backend/app/acceptance/guardrails.py",
    "backend/app/acceptance/fixtures.py",
    "backend/app/acceptance/harness.py",
    "backend/app/acceptance/report.py",
    "backend/app/api/deps.py",
    "backend/app/api/routers/system.py",
    "backend/app/api/routers/models.py",
    "backend/app/api/routers/training.py",
    "backend/app/api/routers/prediction.py",
    "backend/app/api/routers/evaluation.py",
    "backend/app/api/routers/history.py",
    "backend/app/api/routers/upload.py",
    "backend/app/api/routers/status.py",
    "backend/app/api/routers/acceptance.py",
    "backend/app/preprocessing/records.py",
    "backend/app/preprocessing/config.py",
    "backend/app/preprocessing/loader.py",
    "backend/app/preprocessing/validation.py",
    "backend/app/preprocessing/patching.py",
    "backend/app/preprocessing/patch_manifest.py",
    "backend/app/preprocessing/normalization.py",
    "backend/app/preprocessing/splitting.py",
    "backend/app/preprocessing/augmentation.py",
    "backend/app/preprocessing/raster_io.py",
    "backend/app/preprocessing/pipeline.py",
    "backend/app/visualization/__init__.py",
    "backend/app/visualization/records.py",
    "backend/app/visualization/backends.py",
    "backend/app/visualization/colormap.py",
    "backend/app/visualization/statistics.py",
    "backend/app/visualization/inspection.py",
    "backend/app/visualization/bands.py",
    "backend/app/visualization/overlays.py",
    "backend/app/visualization/patches.py",
    "backend/app/visualization/plotting.py",
    "backend/app/visualization/reports.py",
    "backend/app/visualization/qc.py",
    "backend/app/visualization/manifest.py",
    "backend/app/visualization/session.py",
    "backend/app/visualization/exporters.py",
    "backend/scripts/_dataset_cli.py",
    "backend/scripts/download_cloudsen12.py",
    "backend/scripts/download_on_cloud_n.py",
    "backend/scripts/verify_datasets.py",
    "backend/app/models/_torch.py",
    "backend/app/models/config.py",
    "backend/app/models/metadata.py",
    "backend/app/models/artifact.py",
    "backend/app/models/initialization.py",
    "backend/app/models/base.py",
    "backend/app/models/blocks.py",
    "backend/app/models/unet.py",
    "backend/app/models/attention_unet.py",
    "backend/app/models/comparison.py",
    "backend/app/models/summary.py",
    "backend/app/models/registry.py",
    "backend/app/models/factory.py",
    "backend/app/utils/hashing.py",
    "backend/app/training/config.py",
    "backend/app/training/seed.py",
    "backend/app/training/optimizer.py",
    "backend/app/training/scheduler.py",
    "backend/app/training/loss.py",
    "backend/app/training/metadata.py",
    "backend/app/training/logging.py",
    "backend/app/training/checkpoint.py",
    "backend/app/training/callbacks.py",
    "backend/app/training/engine.py",
    "backend/app/training/experiment.py",
    "backend/app/training/lifecycle.py",
    "backend/app/training/artifact.py",
    "backend/app/training/trainer.py",
    "backend/app/evaluation/config.py",
    "backend/app/evaluation/confusion.py",
    "backend/app/evaluation/records.py",
    "backend/app/evaluation/metrics.py",
    "backend/app/evaluation/aggregation.py",
    "backend/app/evaluation/runner.py",
    "backend/app/evaluation/stratification.py",
    "backend/app/evaluation/summary.py",
    "backend/app/evaluation/report.py",
    "backend/app/evaluation/serialization.py",
    "backend/app/evaluation/binary.py",
    "backend/app/failure_analysis/taxonomy.py",
    "backend/app/failure_analysis/config.py",
    "backend/app/failure_analysis/records.py",
    "backend/app/failure_analysis/pixel_analysis.py",
    "backend/app/failure_analysis/sample_analysis.py",
    "backend/app/failure_analysis/ranking.py",
    "backend/app/failure_analysis/stratification.py",
    "backend/app/failure_analysis/analyzer.py",
    "backend/app/failure_analysis/viz_specs.py",
    "backend/app/failure_analysis/report.py",
    "backend/app/comparison/__init__.py",
    "backend/app/comparison/config.py",
    "backend/app/comparison/guardrails.py",
    "backend/app/comparison/records.py",
    "backend/app/comparison/metrics.py",
    "backend/app/comparison/failures.py",
    "backend/app/comparison/decision.py",
    "backend/app/comparison/runner.py",
    "backend/app/comparison/viz_specs.py",
    "backend/app/comparison/report.py",
    "backend/app/comparison/serialization.py",
    "docs/adr/ADR-0006-baseline-model-selection.md",
    "docs/adr/ADR-0007-training-strategy.md",
    "docs/adr/ADR-0008-evaluation-strategy.md",
    "docs/adr/ADR-0009-confusing-case-analysis.md",
    "docs/adr/ADR-0010-improved-model-selection.md",
    "docs/adr/ADR-0011-model-comparison.md",
    "docs/adr/ADR-0012-experimental-dataset-and-data-pipeline.md",
    "docs/adr/ADR-0013-backend-api.md",
    "docs/adr/ADR-0014-frontend.md",
    "docs/adr/ADR-0015-integration-degraded-recovery.md",
    "docs/adr/ADR-0016-acceptance-harness.md",
    "docs/adr/ADR-0017-deployment-containerization.md",
    "docs/adr/ADR-0018-documentation-and-release-packaging.md",
    "docs/adr/ADR-0019-research-paper-and-evidence-policy.md",
    "paper/README.md",
    "paper/00_RESEARCH_PAPER.md",
    "paper/01_LITERATURE_REVIEW.md",
    "paper/02_COMPARISON_TABLE.md",
    "paper/03_ABLATION_TEMPLATE.md",
    "paper/04_RESULTS.md",
    "paper/references.bib",
    "docs/models/README.md",
    "docs/models/improved_model.md",
    "docs/training/README.md",
    "docs/evaluation/README.md",
    "docs/failure_analysis/README.md",
    "docs/comparison/README.md",
    "docs/comparison/real_experiment_cloudsen12.md",
    "docs/integration/README.md",
    "docs/acceptance/README.md",
    "docs/deployment/README.md",
    "docs/README.md",
    "docs/MANIFEST.md",
    "docs/install/README.md",
    "docs/user_guide/README.md",
    "docs/developer_guide/README.md",
    "docs/api/README.md",
    "docs/planning/10_DOCUMENTATION_AUDIT.md",
    "backend/scripts/preprocess.py",
    "backend/scripts/split_dataset.py",
    "backend/scripts/eda_report.py",
    "backend/scripts/model_info.py",
    "backend/scripts/model_compare.py",
    "backend/scripts/train_smoke.py",
    "backend/scripts/evaluate.py",
    "backend/scripts/analyze_failures.py",
    "backend/scripts/compare_models.py",
    "backend/scripts/prepare_dataset.py",
    "backend/scripts/validate_dataset.py",
    "backend/scripts/acquire_cloudsen12.py",
    "backend/scripts/run_real_comparison.py",
    "backend/scripts/serve_api.py",
    "backend/scripts/run_acceptance.py",
    "backend/scripts/verify_deployment.py",
    "backend/scripts/generate_api_docs.py",
    "data/manifests/datasets.yaml",
    "data/metadata/sentinel2_bands.md",
    "docs/datasets/licenses.md",
    "docs/datasets/experimental_pipeline.md",
    "frontend/package.json",
    "docker/docker-compose.yml",
    "docker/backend.Dockerfile",
    "docker/frontend.Dockerfile",
    "docker/requirements-backend.txt",
    "docker/nginx.conf.template",
    "docker/README.md",
    ".dockerignore",
    ".github/workflows/ci-backend.yml",
    ".github/workflows/ci-frontend.yml",
    ".gitignore",
    "README.md",
]


@pytest.mark.parametrize("rel_dir", REQUIRED_DIRS)
def test_required_directory_exists(rel_dir: str) -> None:
    """Each required scaffold directory is present."""
    path = PROJECT_ROOT / rel_dir
    assert path.is_dir(), f"Missing required directory: {rel_dir}"


@pytest.mark.parametrize("rel_file", REQUIRED_FILES)
def test_required_file_exists(rel_file: str) -> None:
    """Each required scaffold file is present."""
    path = PROJECT_ROOT / rel_file
    assert path.is_file(), f"Missing required file: {rel_file}"
