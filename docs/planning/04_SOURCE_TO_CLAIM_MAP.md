...
|---|--------------------|--------|----------------------------------|--------|
| C-1 | On Cloud N provides Sentinel-2 imagery with binary cloud labels (B02,B03,B04,B08). | DrivenData challenge page. | Confirm band list, label semantics, licence, access method. | NOT YET VERIFIED |
| C-2 | CloudSEN12 provides global 13-band Sentinel-2 patches with **multi-class** labels (clear / thick cloud / thin cloud / cloud shadow) suitable for O2 stratification. | Aybar et al., *CloudSEN12*, Scientific Data 9:782 (2022) + dataset host (HuggingFace/Zenodo/GEE). | Confirm class definitions, band availability (incl. B10 cirrus, B11/B12 SWIR), patch size, licence, snow-region coverage. | NOT YET VERIFIED |
