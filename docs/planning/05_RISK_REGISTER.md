...
| **R-14** | **Dataset availability & licensing** — CloudSEN12 / On Cloud N host, version, or licence may restrict download or redistribution of derived masks. | M | H | **Licences verified (M3):** CloudSEN12 = CC0-1.0 (redistribution permitted); On Cloud N = competition terms, **redistribution PROHIBITED** — raw data kept git-ignored, never redistributed. Access routes documented (tacoreader/HF; DrivenData registration). Availability of the actual download still pending at first fetch. **M12:** availability check + readiness gate implemented; real data still **NOT PRESENT** (rasterio/tacoreader not installed) — pipeline never downloads or bypasses access controls. | PM (Data Lead) | Mitigating |
