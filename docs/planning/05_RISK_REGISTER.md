...
| **R-03** | **Sentinel-2 storage requirements** — full CloudSEN12 is tens of GB; laptop storage limited. | H | M | Use hand-labelled / high-quality subset + region-stratified sampling; tiled/streamed download; record exact subset + size budget (A-06). **M12:** deterministic curated subset (`subset_size` bounded) + raw/processed kept git-ignored; readiness pipeline records exact subset/selection hash. | PM (Data Lead) | Mitigating |
...
| **R-14** | **Dataset availability & licensing** — CloudSEN12 / On Cloud N host, version, or licence may restrict download or redistribution of derived masks. | M | H | **Licences verified (M3):** CloudSEN12 = CC0-1.0 (redistribution permitted); On Cloud N = competition terms, **redistribution PROHIBITED** — raw data kept git-ignored, never redistributed. Access routes documented (tacoreader/HF; DrivenData registration). Availability of the actual download still pending at first fetch. **M12:** availability check + readiness gate implemented; pipeline never bypasses access controls. **First real acquisition DONE (2026-08-20):** a bounded CloudSEN12+ L1C subset (CC0) fetched via **tacoreader 0.6.5** into git-ignored raw paths, checksummed, and passed the readiness gate — access route confirmed working. | PM (Data Lead) | Mitigating |
...
| **R-16** | **Thin-cloud class imbalance** — thin-cloud (and haze-like) pixels are rare vs clear/thick, biasing the model and inflating pixel accuracy. | H | H | Class-weighted / Dice loss, targeted sampling of thin-cloud tiles, report per-class IoU/F1 (never rely on pixel accuracy), monitor thin-cloud recall as a guardrail. **M12:** class-distribution report surfaces thin cloud explicitly + flags severe imbalance; readiness gate requires thin-cloud presence; subset selection + stratified group split guarantee thin-cloud/cloud-shadow coverage in every split (distribution never silently altered). **First real run (2026-08-20):** on the bounded CloudSEN12+ subset, Attention U-Net **consistently improved thin-cloud** (IoU mean +0.050, recall & FN better across 3 seeds) — first real evidence the attention mechanism helps the imbalanced thin-cloud class (with a cloud-shadow trade-off; overall MIXED, not AC-4). | PM (ML Lead) | Open |
| **R-17** | **Annotation quality** — CloudSEN12 automatic/scribble labels and thin-cloud boundaries are noisy; On Cloud N labels may disagree. | M | M | Prefer hand-labelled high-quality subset; document label provenance/quality tier in manifest; note label noise as a limitation in results. **M12:** experimental dataset record + artifact capture observed provenance/versions (UNKNOWN/NOT_VERIFIED kept explicit); validation gates check label validity/range before an experiment is READY. | PM (Data Lead) | Open |
